	...
	<?php 
		...
		if(isset($_POST['btnDelete'])){
			...
			if($stmt->prepare($sql_query)) {	
				...
				$stmt->execute();
				// store result 
				$delete_video_result = $stmt->store_result();
				$stmt->close();
			}
			...
			if($stmt_menu->prepare($sql_query)) {	
				...
				$stmt_menu->bind_result($video_data['video']);
			}
			
			// delete all menu image files from directory
			while($stmt_menu->fetch()){
				$menu_video = $video_data[Menu_video];
				$delete_video = unlink("$menu_video");
			}
			...
			if($delete_video_result && $delete_video_result){
