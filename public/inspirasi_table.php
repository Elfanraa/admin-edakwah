...
<div id="content" class="container col-md-12">
	<?php 
		...
		if(empty($keyword)){
			$sql_query = "SELECT *
				FROM inspirasi  
				ORDER BY judul_inspirasi DESC";
		}else{
			$sql_query = "SELECT * 
				FROM inspirasi 
				WHERE judul_inspirasi LIKE ? 
				ORDER BY judul_inspirasi DESC";
		}
		...
		if($stmt_paging ->prepare($sql_query)) {
			...
			$stmt_paging ->bind_result($data['id_inspirasi'], 
					$data['judul_inspirasi'], 
					$data['isi_inspirasi']
					);
	...
	?>
	<h1>Cerita Inspirasi tidak ditemukan
		<a href="add-inspirasi.php">
			<button class="btn btn-danger">Add Cerita Inspirasi</button>
		</a>
	</h1>
	<hr />
		...
		$no=1;
			...
			<tr>
				...
				<td width="1%">
					...
					<a href="delete-inspirasi.php?id=<?php echo $data['id_inspirasi'];?>">
						Delete
					</a>
				</td>
			</tr>
	...
	<h4>
		<?php 
			...
			$function->doPages($offset, 'inspirasi.php', '', $total_records, $keyword);
		?>
	</h4>
